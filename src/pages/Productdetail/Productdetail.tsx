...
interface ProductDetail {
  productDetailId: string; // ProductDetail ID
  productId: string; // Product ID thật để bình luận
  productName: string;
  description: string;
  images: string[];
...
interface Variant {
  ...
  productId: string; // Product ID thật
    ...
    (async () => {
      try {
        ...
        if (data.length > 0) {
          ...
          const base = data[0];
          console.log('🔍 API Response base:', base); // Debug log
          setProduct({
            productDetailId: base.productDetailId, // ProductDetail ID
            productId: base.productId, // Product ID thật từ API
  ...
  return (
    <div className="product-detail-container">
      ...
      <div className="images-section">
        <img src={`http://localhost:3000${mainImage}`} alt="" className="main-image" />
        ...
        <div className="product-rating">
          ...
          <span className="rating-text">
            {productRating.average > 0 ? (
              <>
                {productRating.average.toFixed(1)} ({productRating.total} đánh giá)
              </>
            ) : (
              'Chưa có đánh giá'
            )}
      ...
      {/* Phần bình luận ngang hàng với sản phẩm */}
      <div className="comment-section-wrapper">
        {product && (
          <CommentSection
            productId={product.productId}
            onRatingUpdate={(average: number, total: number) => {
              console.log('ProductDetail received rating:', { average, total }); // Debug log
              setProductRating({ average, total });
            }}
          />
        )}
      </div>
